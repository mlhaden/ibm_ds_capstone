...
def get_pie_chart(entered_site):
    ...
    if entered_site == 'ALL':
        ...
        fig = px.pie(data, values='class', names='Launch Site', title='Percentage of Total Successful Landings by Site')
        return fig
    else:
        ...
        fig = px.pie(counts, values='count', names='class',
                title=f'Percentage of Successful Landings From Site {entered_site}', labels=['Failure', 'Success'])
        return fig
...
def get_scatter_plot(entered_site, payload_settings):
    if entered_site == 'ALL':
        filtered_df = spacex_df
    else:
        filtered_df = spacex_df[spacex_df['Launch Site'] == entered_site]
    min_pay = payload_settings[0]
    max_pay = payload_settings[1]
    ...
    fig = px.scatter(filtered_df, x='Payload Mass (kg)', y='class', color='Booster Version Category',
                    title='Plot of Landing Success by Payload',
                    labels={'Payload Mass (kg)': 'Payload', 'class': 'Landing Success'})
